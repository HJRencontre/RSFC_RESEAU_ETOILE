...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    ...
    if (typeof id === "string") {
      if (req.method === "GET") {
        const result =
          await sql`SELECT * FROM partner_contacts WHERE is_deleted = false AND id = ${id}`;
      ...
      if (req.method === "DELETE") {
        const result =
          await sql`UPDATE partner_contacts SET is_deleted = true WHERE id = ${id}`;
        const rows = result.rows;

        return res
          .status(200)
          .json("La demande de contact à bien été supprimée.");
      }
    }
  } catch (error) {
