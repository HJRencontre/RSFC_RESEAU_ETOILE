    ...
    try {
        ...
        if (typeof id === 'string') {
            if (req.method === 'GET') {
                ...
                const result = await sql`SELECT * FROM business_segments WHERE is_deleted = false AND id = ${id}`;
            ...
            if (req.method === 'DELETE') {
                ...
                const result    = await sql`UPDATE business_segments SET is_deleted = true WHERE id = ${id}`
                const rows      = result.rows
